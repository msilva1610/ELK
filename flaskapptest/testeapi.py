...
def teste_get():
    r = requests.get('http://192.168.100.20/cliente/')
    print(r.json())

def teste_post01(data):
    r = requests.post('http://192.168.100.20/cliente/',data=data)
    print(r.json())
...
def teste_get_clienteid(id_product):
    r = requests.get('http://192.168.100.20/cliente/{}'.format(id_product))
    print(r.json())
